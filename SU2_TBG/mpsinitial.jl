...
function MPSfg(l, statelist, phySpace, opzo, opfn)
    ...
    Vr = statelist[l]
    MPS[l] = ones(Float64, Rm(Vr => 1) ← phySpace[l] ⊗ Rm(opzo => 1))
    for i = l-1:-1:2
        Vl = inbond[l-i]
        MPS[i] = ones(Float64, Rm(Vl => 1) ← phySpace[i] ⊗ Rm(Vr => 1))
        Vr = Vl
    end
    MPS[1] = ones(Float64, Rm(opfn => 1) ← phySpace[1] ⊗ Rm(Vr => 1))
    #   MPS diagram:
    #  ← o1 ← o2 ← ... ← ol ←
    #    ↑    ↑    ...   ↑
    return MPS
...
function rdMPS!(l, mps, mdim, nsweep)
    for ins = 1:nsweep
        for sl = 1:l-1
            tt = rand(ComplexF64, phySpace[sl]' ⊗ phySpace[sl+1]' ← phySpace[sl]' ⊗ phySpace[sl+1]')
            q, r = left_orth(tt)
            qb = isometry(domain(q) ← domain(tt))
            @tensor mps2[p1 p2; b1 b2] := mps[sl][b1 p1; b0] * mps[sl+1][b0 p2; b2]
            mps2n = permute(q * qb * mps2, ((3,1),(2,4)))
            U, S, V, ϵ = svd_trunc(permute(mps2n, ((1, 2), (3, 4))); trunc=truncrank(mdim))
        ...
        for sl = l-1:-1:1
            ...
            @tensor mps2[p1 p2; b1 b2] := mps[sl][b1 p1; b0] * mps[sl+1][b0 p2; b2]
            mps2n = q * qb * mps2
            U, S, V, ϵ = svd_trunc(permute(mps2n, ((3, 1), (2, 4))); trunc=truncrank(mdim))
