...
function ltrt!(lt, rt, m, mf, l)
    lt[1] = isometry(ComplexF64, zb ← zb)
    for il = 2:l+1
        lt[il] = con_left(lt[il-1], m[il-1]', mf[il-1])
    end
    rt[l+1] = isometry(ComplexF64, nb ← nb)
    for il = l:-1:1
