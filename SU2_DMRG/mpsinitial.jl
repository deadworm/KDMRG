...
function MPSfg(l, statelist, phySpace, opzo, opfn)
    ...
    Vr = statelist[l]
    MPS[l] = TensorMap(ones, Float64, Rm(Vr => 1) ← phySpace ⊗ Rm(opzo => 1))
    for i = l-1:-1:2
        Vl = inbond[l-i]
        MPS[i] = TensorMap(ones, Float64, Rm(Vl => 1) ← phySpace ⊗ Rm(Vr => 1))
        Vr = Vl
    end
    MPS[1] = TensorMap(ones, Float64, Rm(opfn => 1) ← phySpace ⊗ Rm(Vr => 1))
    #   MPS diagram:
    #  ← o1 ← o2 ← ... ← ol ←
    #    ↑    ↑    ...   ↑
    return MPS
